...
class CoreModuleTesting(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.main_key = lc.Enc.gen_key()
        self.ins1 = lc.Enc(message=self.message, mainkey=self.main_key)
        self.string_message = self.ins1.encrypt()
        self.bytes_message = self.ins1.encrypt(get_bytes=True)
        self.ins2 = lc.Dec(message=self.bytes_message, mainkey=self.main_key)
        self.h = Size.HMAC
    ...
    def test_HMAC_MismatchError(self):
        ...
        with self.assertRaises(exceptions.fixed.MessageTamperingError):
            lc.Dec(message=tampered_message, mainkey=self.main_key)
    ...
    def test_key_length(self):
        self.assertEqual(self.k, bytes.fromhex(lc.Enc.gen_key()).__len__())

    def test_key_type(self):
        self.assertIs(str, type(lc.Enc.gen_key()))
